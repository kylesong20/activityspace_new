<template>
  <div class="app-container">
    ...
    <!-- 工具条 -->
    <div>
      <el-button v-if="hasPerm('role.add')" size="mini" type="primary" @click="addUser()">添加</el-button>
      <el-button v-if="hasPerm('role.remove')" size="mini" type="danger" @click="removeRows()">批量删除</el-button>
    </div>
      ...
      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="scope">
          <router-link :to="'/acl/role/distribution/'+scope.row.id">
            <el-button v-if="hasPerm('role.acl')" icon="el-icon-info" size="mini" type="info" />
          </router-link>
          <router-link :to="'/acl/role/update/'+scope.row.id">
            <el-button v-if="hasPerm('role.update')" icon="el-icon-edit" size="mini" type="primary" />
          </router-link>
          <el-button
...
<script>
import roleApi from '@/api/activityspace/acl/role'
